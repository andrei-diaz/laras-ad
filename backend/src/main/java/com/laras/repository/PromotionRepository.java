package com.laras.repository;

import com.laras.entity.Promotion;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.time.LocalDate;
import java.util.List;

@Repository
public interface PromotionRepository extends JpaRepository<Promotion, Long> {

    List<Promotion> findByIsActiveTrueOrderByIsFeaturedDescCreatedAtDesc();

    List<Promotion> findByIsActiveTrueAndIsFeaturedTrueOrderByCreatedAtDesc();

    List<Promotion> findByIsActiveTrueAndStartDateLessThanEqualAndEndDateGreaterThanEqualOrderByIsFeaturedDesc(
            LocalDate today1, LocalDate today2);

    List<Promotion> findAllByOrderByCreatedAtDesc();
}
